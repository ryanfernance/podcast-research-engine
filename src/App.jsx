...
export default function App(){
  ...
  const generate=async()=>{
    ...
    const ctx=matched.slice(0,6).map(v=>"- \""+v.title+"\" by "+v.ch+" ("+v.pi+"x, "+fmt(v.views)+" views)").join("\n");
    const gl=isInt?"INTERNAL episode. Doza is a former multi-studio fitness owner who coaches hundreds of studio owners.":"GUEST: "+gName+" — "+gDesc;
    const promptText="You produce the Geronimo Unfiltered Podcast for fitness studio owners. Host: Doza.\n\n"+gl+"\n\nGenerate 3 episode concepts from the proven videos below. For each:\n- title: Punchy, specific title\n- hook: Why someone clicks (one sentence)\n- why_it_works: Why this is pre-validated (reference the source video)\n- beats: 4 concrete content beats to hit\n- opening: First 20 seconds in Doza's voice\n- source: Which video inspired this and its performance\n\nReturn ONLY JSON array:\n[{\"title\":\"\",\"hook\":\"\",\"why_it_works\":\"\",\"beats\":[\"\",\"\",\"\",\"\"],\"opening\":\"\",\"source\":\"\"}]\n\nPROVEN VIDEOS:\n"+ctx;
    try{
